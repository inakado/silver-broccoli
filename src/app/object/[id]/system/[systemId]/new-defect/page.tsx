...
export default function NewDefectPage({ params }: { params: { id: string; systemId: string } }) {
  ...
  const [markedPlan, setMarkedPlan] = useState<string | null>(null)
            ...
            <div className="mt-1">
              {photo ? (
                <div className="relative aspect-video">
                  <img src={photo || "/placeholder.svg"} alt="Captured defect" className="w-full h-full object-cover rounded-lg" />
                  <Button type="button" variant="secondary" size="sm" className="absolute bottom-2 right-2" onClick={() => setPhoto(null)}>
                    Изменить фото
                  </Button>
                </div>
              ) : (
                <div className="relative">
                  <input id="photo" type="file" accept="image/*" capture="environment" onChange={handleFileCapture} className="sr-only" />
                  <label htmlFor="photo" className="flex items-center justify-center w-full h-40 border-2 border-dashed rounded-lg cursor-pointer hover:bg-gray-50">
          ...
          <Sheet>
            ...
            <SheetContent  className="h-3/4">
              ...
              <div className="flex flex-col items-center justify-center mt-4">
                <canvas ref={canvasRef} width={500} height={500} className="w-full h-auto border rounded-lg" onClick={handleCanvasClick} />
